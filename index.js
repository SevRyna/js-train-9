...
function findIndexByProperty(arr, name) {
  ...
  const found = arr.findIndex(isName);
...
function getPositiveArrayValues(arr) {
  ...
  const filter = array.filter((val) => val > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  //console.log(arr.length-startIdx);
  arr.splice(startIdx, arr.length - startIdx);
  //console.log(arr);
...
function findLongestWord(arr) {
  ...
  const initialValue = "not found";
  const foundInitial = arr.reduce(
    (accumulator, currentValue) =>
      accumulator.length < currentValue.length ? currentValue : currentValue,
    initialValue
  );
...
function capitalizeWords(arr) {
  ...
  // Використовуємо метод map() для перетворення кожного слова в рядку з першою великою літероюconst map = arr.map(x => x.charAt(0).toUpperCase() + x.slice(1));
  // Перетворюємо першу літеру слова у верхній регістр да додамо до неї всі символи слова крім першого
  const map = arr.map((x) => x.charAt(0).toUpperCase() + x.slice(1));
  //Повертаємо слово
...
function findLastNegativeNumberIndex(arr) {
  ...
  const isMinus = (element) => element < 0;
  return arr.findLastIndex(isMinus);
}
